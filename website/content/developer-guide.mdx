...
Install the following tools before you begin or use a [Dev Container](#optional-dev-container-setup). These are verified requirements from the Armada source code:
...
This project has a [Dev Container](https://code.visualstudio.com/docs/devcontainers/containers) that includes all of the required tools required for Armada development. To configure git within the devcontainer an env file at `.devcontainer/devcontainer.env` can be used including a [ssh signing key](https://docs.github.com/en/enterprise-cloud@latest/authentication/connecting-to-github-with-ssh/generating-a-new-ssh-key-and-adding-it-to-the-ssh-agent).

```
# example devcontainer.env
GIT_USER_NAME=Hugh.Man
GIT_USER_EMAIL=hugh.man@example.com
GIT_SIGNING_KEY=ssh-123 somthingforwarded
```
